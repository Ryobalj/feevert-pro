...
const AdminEmailAccountsPage = () => {
  ...
  const handleCreate = async (e) => {
    ...
    if (!form.email_address || !form.imap_host || !form.imap_password) {
      setError('Email address, IMAP host, and IMAP password are required')
      return
    }
    setCreating(true)
    try {
      ...
      setError(typeof detail === 'object' ? JSON.stringify(detail) : 'Failed to create account')
    } finally {
      setCreating(false)
    }
  }
  ...
  const handleDelete = async (account) => {
    if (!window.confirm(`Remove mailbox ${account.email_address}? Its stored emails stay, but syncing stops.`)) return
    try {
  ...
  const handleSyncNow = async (account) => {
    ...
    try {
      ...
      alert(res.data.success ? `Synced: ${res.data.saved || 0} new email(s)` : `Failed: ${res.data.error || 'unknown error'}`)
      loadData()
    } catch (err) {
      alert('Sync failed')
    } finally {
  ...
  return (
    <div className="container-main py-8 md:py-12 max-w-3xl">
      <div className="mb-6">
        <h1 className="text-2xl md:text-3xl font-extrabold text-white">Staff Mailboxes</h1>
        <p className="text-white/40 text-sm mt-1">
          Connect each staff member's own @feevert.co.tz mailbox, or leave "staff member" blank for a shared inbox everyone can see.
        </p>
      </div>

      <form onSubmit={handleCreate} className="glass-card p-6 mb-6 space-y-3">
        <h2 className="text-sm font-bold text-white/70 uppercase tracking-wide mb-2">Connect a Mailbox</h2>
        <div className="grid sm:grid-cols-2 gap-3">
          <input type="email" value={form.email_address} onChange={(e) => handleChange('email_address', e.target.value)}
            placeholder="staff@feevert.co.tz"
        ...
        <p className="text-xs text-white/30">SMTP defaults to the same host/password as IMAP unless you set them separately in the account's edit view later.</p>
        {error && <p className="text-red-400 text-sm">{error}</p>}
        <button type="submit" disabled={creating} className="btn-primary text-sm disabled:opacity-50">
          {creating ? 'Connecting...' : 'Connect Mailbox'}
        </button>
      </form>
      ...
      {loading ? (
        ...
        <div className="space-y-3">
                ...
                <div>
                  <h3 className="font-bold text-white flex items-center gap-2">
                    ...
                    {!account.owner_username && (
                      <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-emerald-500/15 text-emerald-400">shared</span>
                    )}
                  </h3>
                  <p className="text-xs text-white/40 mt-1">
                    {account.owner_username ? `Owner: ${account.owner_username}` : 'Visible to all staff'} · {account.provider.toUpperCase()}
                    {account.last_synced_at && ` · Last synced ${new Date(account.last_synced_at).toLocaleString()}`}
                  </p>
                  {account.last_sync_error && (
                    <p className="text-xs text-red-400 mt-1">Last error: {account.last_sync_error.slice(0, 150)}</p>
                  )}
                </div>
          ...
          {accounts.length === 0 && (
            <div className="glass-card p-10 text-center text-white/30">No mailboxes connected yet.</div>
          )}
        </div>
      )}
    </div>
