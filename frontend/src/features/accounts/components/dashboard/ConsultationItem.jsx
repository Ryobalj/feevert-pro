...
const ConsultationItem = ({ consultation, darkMode, showClient = false }) => {
  const getStatusColor = (status) => {
    const colors = {
      ...
      in_progress: darkMode ? 'bg-blue-900/30 text-blue-300' : 'bg-blue-100 text-blue-700',
    ...
    return colors[consultation.status] || (darkMode ? 'bg-gray-700 text-gray-300' : 'bg-gray-100 text-gray-700')
  }

  return (
    <Link to={`/consultations/${consultation.id}`} className={`block p-4 rounded-lg transition-colors ${darkMode ? 'bg-gray-700/50 hover:bg-gray-700' : 'bg-gray-50 hover:bg-gray-100'}`}>
      <div className="flex justify-between items-start">
        <div>
          ...
          <p className={`text-sm ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            {consultation.preferred_date ? new Date(consultation.preferred_date).toLocaleDateString() : 'Date TBD'}
          </p>
        </div>
