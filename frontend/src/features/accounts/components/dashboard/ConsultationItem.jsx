// src/features/accounts/components/dashboard/ConsultationItem.jsx
...
const ConsultationItem = ({ consultation, darkMode, showClient = false }) => {
  ...
  const statusConfig = {
    confirmed: { 
      ...
      icon: '✅', 
      label: t('consultation.status.confirmed') 
    },
    pending: { 
      badge: 'bg-amber-500/15 text-amber-400 border-amber-500/20', 
      icon: '⏳', 
      label: t('consultation.status.pending') 
    },
    in_progress: { 
      badge: 'bg-blue-500/15 text-blue-400 border-blue-500/20', 
      icon: '🔄', 
      label: t('consultation.status.in_progress') 
    },
    completed: { 
      badge: 'bg-purple-500/15 text-purple-400 border-purple-500/20', 
      icon: '✔️', 
      label: t('consultation.status.completed') 
    },
    cancelled: { 
      badge: 'bg-red-500/15 text-red-400 border-red-500/20', 
      icon: '❌', 
      label: t('consultation.status.cancelled') 
    },
  }
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <p className="font-semibold text-white text-sm group-hover:text-emerald-400 transition-colors duration-300 truncate">
                {consultation?.service_name || t('consultation.service')}
              </p>
            </div>
