...
const ConsultationList = () => {
  ...
  // ✅ Badilisha statuses kuwa na translation
  const getStatuses = () => [
    { value: 'all', icon: '📋', label: t('status.all') },
    { value: 'pending', icon: '⏳', label: t('status.pending') },
    { value: 'confirmed', icon: '✅', label: t('status.confirmed') },
    { value: 'in_progress', icon: '🔄', label: t('status.in_progress') },
    { value: 'completed', icon: '✔️', label: t('status.completed') },
    { value: 'cancelled', icon: '❌', label: t('status.cancelled') },
  ]
  ...
  const getStatusLabel = (status) => {
    ...
    return found ? found.label : status?.replace('_', ' ') || t('status.unknown')
  }
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <h1 className="text-2xl md:text-3xl lg:text-4xl font-extrabold text-white mb-2">
              {t('list.my_consultations')} <span className="gradient-text">{t('list.consultations')}</span>
            </h1>
            <p className="text-white/40 text-sm">{t('list.track_requests')}</p>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <Link to="/request-consultation" className="group relative inline-flex items-center gap-2 bg-emerald-500 hover:bg-emerald-400 text-white px-6 py-3 rounded-full font-semibold text-sm shadow-lg shadow-emerald-500/20 transition-all overflow-hidden">
              ...
              <span className="relative z-10">{t('list.new_request')}</span>
            </Link>
          </motion.div>
        </div>
        ...
        {consultations.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="glass px-4 py-2 rounded-full text-sm text-white/50">
              <span className="text-white font-semibold">{stats.total}</span> {t('list.total')}
            </div>
            {stats.pending > 0 && (
              <div className="glass px-4 py-2 rounded-full text-sm text-amber-400/70">
                <span className="text-amber-400 font-semibold">{stats.pending}</span> {t('status.pending').toLowerCase()}
              </div>
            )}
            {stats.confirmed > 0 && (
              <div className="glass px-4 py-2 rounded-full text-sm text-emerald-400/70">
                <span className="text-emerald-400 font-semibold">{stats.confirmed}</span> {t('status.confirmed').toLowerCase()}
              </div>
            )}
          </motion.div>
        )}
          ...
          {filteredConsultations.length > 0 ? (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="grid sm:grid-cols-2 gap-2 text-sm mb-3">
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-white/50 text-xs">{t('list.date_label')}:</span>
                              ...
                              <div className="flex items-center gap-2">
                                ...
                                <span className="text-white/50 text-xs">{t('list.assigned_label')}:</span>
                                <span className="text-white/70 text-sm font-medium">
                                  {consultation.assigned_to_name || t('detail.pending_assignment')}
                                </span>
                              </div>
                            </div>
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <h3 className="text-xl font-bold text-white mb-2">
                {filter !== 'all' 
                  ? t('list.no_filtered_results', { status: getStatusLabel(filter) })
                  : t('list.no_results')}
              </h3>
              <p className="text-white/40 max-w-sm mx-auto mb-6">
                {filter !== 'all' 
                  ? t('list.no_filtered_message', { status: getStatusLabel(filter).toLowerCase() })
                  : t('list.no_results_message')}
              </p>
              <Link to="/request-consultation" className="btn-primary btn-lg inline-flex items-center gap-2">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                </svg>
                {t('list.request_now')}
