...
class Review(BaseModel):
    """
    Customer reviews and ratings for services AND products
    """
    RATING_CHOICES = (
        (1, '1 Star - Poor'),
        (2, '2 Stars - Fair'),
    ...
    # 🆕 Generic ForeignKey - can reference ConsultationRequest, Booking, OR Product
    content_type = models.ForeignKey(ContentType, on_delete=models.CASCADE, null=True, blank=True)
    object_id = models.PositiveIntegerField(null=True, blank=True)
    content_object = GenericForeignKey('content_type', 'object_id')
    
    # Old FK fields (keep for backward compatibility)
...
class ReviewHelpfulVote(BaseModel):
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=['review', 'is_helpful'])]
